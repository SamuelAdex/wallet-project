...
const WalletPage = () => {
  ...
  return (
    <div className='grid place-items-center p-3 md:h-[100vh] bg-[#000] relative '>
        ...
        {/* {isLoading && <Loader />} */}
        <div className='md:w-[884px] w-full py-10 md:px-14 px-5 md:space-y-8 grid place-items-center md:mt-auto mt-14'>
            <div className=''>
                {/* {userData?.wallets[0]} */}
            ...
            <div className='flex items-center justify-center'>
                <div className='text-gray-300 md:text-2xl cursor-pointer'>{wallet?.address && wallet?.address.slice(0,10)+"..."+wallet.address.slice(wallet?.address.length - 10, wallet?.address.length)}</div>
                <IoCopyOutline onClick={()=> copyText(wallet?.address)} className='text-3xl text-white cursor-pointer' />
            </div>

            <div className='md:w-[543px] w-full md:space-y-6 space-y-3 md:mt-auto mt-6'>    
                {isWalletLoading ? (
